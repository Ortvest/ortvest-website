...
import { ArrowRight, Briefcase, Code, Lock, Megaphone, Palette } from 'lucide-react';
...
export function Cases() {
  ...
  const [allSubfilter, setAllSubfilter] = useState<'all' | 'logo' | 'web' | 'brand' | 'mobile'>('all');
  ...
  let filteredCases =
    activeFilter === 'all'
      ? cases
      : cases.filter((c) => c.category === activeFilter || c.category === 'designAndDevelopment');
  ...
  if (activeFilter === 'all' && allSubfilter !== 'all') {
    if (allSubfilter === 'logo' || allSubfilter === 'brand') {
      ...
      filteredCases = filteredCases.filter((c) => c.designSubcategory === 'web' || c.developmentSubcategory === 'web');
              ...
              {(['all', 'design', 'development'] as const).map((filter) => (
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-sm font-medium transition ${
                    activeFilter === filter ? 'bg-black text-white' : 'bg-black/5 text-black/70 hover:bg-black/10'
                  }`}>
                  {t(`filter.${filter}`)}
                </button>
              ))}
              ...
              <div className="flex flex-wrap items-center gap-2 pl-4">
                ...
                <button
                  ...
                  className={`rounded-full px-3 py-1.5 text-xs font-medium transition ${
                    allSubfilter === 'all' ? 'bg-accent text-black' : 'bg-black/5 text-black/60 hover:bg-black/10'
                  }`}>
                  {t('filter.all')}
                </button>
                <button
                  ...
                  className={`rounded-full px-3 py-1.5 text-xs font-medium transition ${
                    allSubfilter === 'logo' ? 'bg-accent text-black' : 'bg-black/5 text-black/60 hover:bg-black/10'
                  }`}>
                ...
                <button
                  ...
                  className={`rounded-full px-3 py-1.5 text-xs font-medium transition ${
                    allSubfilter === 'web' ? 'bg-accent text-black' : 'bg-black/5 text-black/60 hover:bg-black/10'
                  }`}>
                ...
                <button
                  ...
                  className={`rounded-full px-3 py-1.5 text-xs font-medium transition ${
                    allSubfilter === 'brand' ? 'bg-accent text-black' : 'bg-black/5 text-black/60 hover:bg-black/10'
                  }`}>
                ...
                <button
                  ...
                  className={`rounded-full px-3 py-1.5 text-xs font-medium transition ${
                    allSubfilter === 'mobile' ? 'bg-accent text-black' : 'bg-black/5 text-black/60 hover:bg-black/10'
                  }`}>
              ...
              <div className="flex flex-wrap items-center gap-2 pl-4">
                ...
                {(['all', 'web', 'mobile'] as const).map((subfilter) => (
                  <button
                    ...
                    className={`rounded-full px-3 py-1.5 text-xs font-medium transition ${
                      devSubfilter === subfilter ? 'bg-accent text-black' : 'bg-black/5 text-black/60 hover:bg-black/10'
                    }`}>
